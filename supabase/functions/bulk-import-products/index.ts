  ...
  try {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      try {
        ...
        const { data: slugResult } = await supabaseClient
          .rpc('generate_unique_slug', { product_name: rowData['Product Name'] || '' });
        
        if (slugResult) {
          productData.slug = slugResult;
        } else {
          // Fallback slug generation
          productData.slug = (rowData['Product Name'] || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') + '-' + Date.now();
        }
